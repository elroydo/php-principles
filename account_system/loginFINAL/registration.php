    ...
    <body>
        ...
        <?php
            ...
            if ((!empty($_POST['name']) AND ($_POST['sname'])AND ($_POST['username'])AND ($_POST['password']))) { 
                ...
                if (ctype_alpha($firstName) AND ctype_alpha($surName) AND preg_match('/^(?=.*[A-Za-z]).{4,20}$/', $firstName) AND preg_match('/^(?=.*[A-Za-z]).{4,20}$/', $surName) AND filter_var($username, FILTER_VALIDATE_EMAIL) AND preg_match('/^(?=.*\d)(?=.*[A-Za-z])[0-9A-Za-z!@#$%].{8,12}$/', $password)) {
        ...
        <br>
        <p>
            Firstname: Alphabetical, ranging from 4-20 characters.<br>
            Lastname: Alphabetical, ranging from 4-20 characters.<br>
